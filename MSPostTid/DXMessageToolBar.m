...
//附加视图高度
#define CUST_HEIGHT 200
#define COLOR_MORE [UIColor colorWithRed:0.85 green:0.85 blue:0.85 alpha:1];

@interface DXMessageToolBar()
<
//DXFaceDelegate,
UITextViewDelegate
>
...
- (void)setupSubviews
{
    ...
    self.styleChangeButton.showsTouchWhenHighlighted = YES;
    ...
    [self.moreButton setImage:[UIImage imageNamed:@"favour"] forState:UIControlStateNormal];
    //[self.moreButton setImage:[UIImage imageNamed:@"chatBar_moreSelected"] forState:UIControlStateHighlighted];
    [self.moreButton setImage:[UIImage imageNamed:@"like_pressed"] forState:UIControlStateSelected];
    ...
    [self.faceButton setImage:[UIImage imageNamed:@"unlike_normal"] forState:UIControlStateNormal];
    //[self.faceButton setImage:[UIImage imageNamed:@"chatBar_faceSelected"] forState:UIControlStateHighlighted];
    [self.faceButton setImage:[UIImage imageNamed:@"tread"] forState:UIControlStateSelected];
    ...
    _inputTextView.placeHolder = @"输入内容"; //unitized(@"place.message");
...
//    self.recordButton.backgroundColor = [UIColor colorWithRed:0.96 green:0.96 blue:0.96 alpha:1.0];
...
//    self.recordButton.hidden = YES;
    ...
    if(!addtionView){
        ...
        addtionView = [[UIView alloc] initWithFrame:CGRectMake(0,0, self.frame.size.width, CUST_HEIGHT+(kVerticalPadding * 2 + kInputTextViewMinHeight))];
        addtionView.backgroundColor = [UIColor clearColor];
        addtionView.userInteractionEnabled = YES;
        //addtionView.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
        //第0层是什么卵东西
        [self insertSubview:addtionView atIndex:1];
        
        
    }
    
//    if (!self.moreView) {
//        self.moreView = [[DXChatBarMoreView alloc] initWithFrame:CGRectMake(0, (kVerticalPadding * 2 + kInputTextViewMinHeight), self.frame.size.width, CUST_HEIGHT) typw:_moreType];
//        self.moreView.backgroundColor = COLOR_MORE;
//        self.moreView.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
//    }
    
    // Emoj表情自定义键盘区
    if (!self.faceView) { // faceView坐标（0，46，width，200）;
        self.faceView = [[DXFaceView alloc] initWithFrame:CGRectMake(0, (kVerticalPadding * 2 + kInputTextViewMinHeight), self.frame.size.width, CUST_HEIGHT)];
        [(DXFaceView *)self.faceView setDelegate:self];
        self.faceView.backgroundColor = COLOR_MORE;
...
//    if (!self.isBuddy) {
//        //不能发表情
//        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:nil message:unitized(@"message.forbid") delegate:nil cancelButtonTitle:unitized(@"Cancel") otherButtonTitles: nil];
//        [alert show];
//        return;
//    }
    ...
    switch (tag) {
        ...
        case 1://表情
        {
            if (button.selected) {
                ...
                if (!self.styleChangeButton.selected) { // 当前选中的不是录音
                    [self.inputTextView becomeFirstResponder];
                }
                else {
                    [self willShowBottomView:nil]; // 禁止弹出底部弹框
                }
            }
        }
            break;
...
- (UIView *)toolbarView
{
    if (_toolbarView == nil) {
        ...
        _toolbarView.backgroundColor = COLOR_MORE;
    }
...
{
    //NSLog(@"s-frame-----%@",NSStringFromCGRect(self.superview.frame));
...
- (void)willShowBottomHeight:(CGFloat)bottomHeight
{
    ...
    //执行动画,需要移除吗，这个逻辑怎么那么混乱啊 键盘，录音表情self.isShowButtomView? 2.2:0
    if (!response && bottomHeight == 0) {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        if (addtionView.subviews.count>1) {
            
            [(UIView *)addtionView.subviews[0] removeFromSuperview];
            
        }
        self.activityButtomView = nil;
        if(self.frame.origin.x == [UIScreen mainScreen].bounds.size.height-46){}
    }];
...
- (void)willShowBottomView:(UIView *)bottomView
{
    ...
    if (![self.activityButtomView isEqual:bottomView]) {
        ...
        if (bottomView) {
            ...
            [addtionView addSubview:bottomView];
...
- (void)willShowKeyboardFromFrame:(CGRect)beginFrame toFrame:(CGRect)toFrame
{
    ...
    {
        //弹出：一定要把self.activityButtomView置为空
...
- (void)willShowInputTextViewToHeight:(CGFloat)toHeight
{
    ...
    if (toHeight == _previousTextViewContentHeight)
    {
        ...
        self.frame = rect;
        addtionView.frame = CGRectMake(addtionView.frame.origin.x, addtionView.frame.origin.y+changeHeight, addtionView.frame.size.width,  addtionView.frame.size.height);
